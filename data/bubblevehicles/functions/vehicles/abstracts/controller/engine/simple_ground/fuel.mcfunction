...
execute if score @s bvehicle.speed = @e[type=marker, tag=btemp.origin, limit=1] buvs.max_speed run scoreboard players add @e[type=marker, tag=btemp.origin, limit=1] bvehicle.fuel 4